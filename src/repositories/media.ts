...
import type { DataListResult, Media } from '../type'
import type { QueryResult } from 'pg'

const MEDIA_TABLE = 'media'

const MEDIA_COLUMN_MAP: Record<string, string> = {
    id: 'id',
    ownerId: 'owner_id',
    mimeType: 'mime_type',
...
const toMedia = (row: Record<string, unknown>): Media => {
    return {
        id: row.id as string,
        ...
        metadata: row.metadata as Media['metadata'],
...
/**
 * Query media with pagination and ordering
 * @param filters filter criteria
 * @param page page number
 * @param pageSize items per page
 * @param sortBy sort field
 * @param order sort direction
 * @returns media list and total count
 */
export const queryMedia = async (
    ...
    for (const [key, value] of Object.entries(filters)) {
        ...
        const col = MEDIA_COLUMN_MAP[key]
    ...
    const orderCol = MEDIA_COLUMN_MAP[sortBy]
    if (!orderCol) throw 'INVALID_SORT_BY'

    const countSql = `SELECT COUNT(*)::int AS total FROM ${MEDIA_TABLE} ${whereClause}`
    ...
    return {
        list: res.rows.map((row) => toMedia(row)),
