/**
 * Chat topic repository
 */
import { pgClient, parseError } from '../modules/pg'
import type { ChatTopic, DataListResult } from '../type'
import type { QueryResult } from 'pg'

const CHAT_TOPIC_TABLE = 'chat_topics'

const CHAT_TOPIC_COLUMN_MAP: Record<string, string> = {
    id: 'id',
    summary: 'summary',
    participants: 'participants',
    soulId: 'soul_id',
    conversationId: 'conversation_id',
    startedAt: 'started_at',
    endedAt: 'ended_at',
    metadata: 'metadata',
    createdAt: 'created_at',
    updatedAt: 'updated_at',
}

const toChatTopic = (row: Record<string, unknown>): ChatTopic => {
    return {
        id: row.id as string,
        summary: row.summary as string,
        embedding: row.embedding as number[],
        participants: row.participants as string[],
        soulId: row.soul_id as string,
        conversationId: row.conversation_id as string,
        startedAt: row.started_at as Date,
        endedAt: row.ended_at as Date,
        metadata: row.metadata as ChatTopic['metadata'],
        createdAt: row.created_at as Date,
        updatedAt: row.updated_at as Date,
    }
}

/**
 * Query chat topics with pagination and ordering
 * @param filters filter criteria
 * @param page page number
 * @param pageSize items per page
 * @param sortBy sort field
 * @param order sort direction
 * @returns chat topic list and total count
 */
export const queryChatTopics = async (
    filters: Record<string, unknown> = {},
    page: number = 1,
    pageSize: number = 20,
    sortBy: string = 'createdAt',
    order: 'asc' | 'desc' = 'desc',
): Promise<DataListResult<ChatTopic>> => {
    if (!pgClient) throw 'POSTGRES_NOT_READY'

    if (!Number.isFinite(page) || page < 1) throw 'INVALID_PAGE'
    if (!Number.isFinite(pageSize) || pageSize <= 0) throw 'INVALID_PAGE_SIZE'
    if (order !== 'asc' && order !== 'desc') throw 'INVALID_ORDER'

    const values: unknown[] = []
    const conditions: string[] = []

    for (const [key, value] of Object.entries(filters)) {
        if (value === undefined) continue

        const col = CHAT_TOPIC_COLUMN_MAP[key]
        if (!col) throw 'INVALID_QUERY_KEYS'

        if (!Array.isArray(value)) {
            conditions.push(`${col} = $${values.length + 1}`)
            values.push(value)
            continue
        }

        const [start, end] = value as [unknown?, unknown?]
        if (start != null && start !== '') {
            conditions.push(`${col} >= $${values.length + 1}`)
            values.push(start)
        }
        if (end != null && end !== '') {
            conditions.push(`${col} <= $${values.length + 1}`)
            values.push(end)
        }
    }

    const whereClause = conditions.length > 0 ? `WHERE ${conditions.join(' AND ')}` : ''

    const orderCol = CHAT_TOPIC_COLUMN_MAP[sortBy]
    if (!orderCol) throw 'INVALID_SORT_BY'

    const countSql = `SELECT COUNT(*)::int AS total FROM ${CHAT_TOPIC_TABLE} ${whereClause}`

    values.push(pageSize, (page - 1) * pageSize)
    const sql = `
        SELECT * FROM ${CHAT_TOPIC_TABLE}
        ${whereClause}
        ORDER BY ${orderCol} ${order === 'asc' ? 'ASC' : 'DESC'}
        LIMIT $${values.length - 1} OFFSET $${values.length}
    `

    let countRes: QueryResult<{ total: number }>
    let res: QueryResult<Record<string, unknown>>
    try {
        [countRes, res] = await Promise.all([
            pgClient.query(countSql, values.slice(0, values.length - 2)),
            pgClient.query(sql, values),
        ])
    } catch (error) {
        throw parseError(error)
    }

    return {
        list: res.rows.map((row) => toChatTopic(row)),
        total: countRes.rows[0]?.total as number,
    }
}
